...
Hierarchy is SERIES → SEASON → EPISODE; FILM/SHORT/TRAILER are roots. Composite foreign keys preserve parent ownership, the mutation RPC validates parent type, and positions are unique per parent. Structure is fixed once created; edits change metadata only. Parent/owner lookups are indexed. All content and replay records cascade with Cinema profile deletion. Account deletion is a support workflow in this repository; deleting an Auth identity alone does not delete public financial users or their Cinema profile. Both draft RPCs additionally require the Auth identity to exist, denying even a still-unexpired token after Auth deletion. The support deletion workflow must remove the Cinema profile to erase drafts/replay metadata. There is no privileged audit or indefinite evidence retention in this draft store.
...
Local browser evidence: 360px gated page and isolated synthetic API responses verified the empty workspace, create request, saving state, saved list, edit metadata, revision-conflict copy with preserved input and conflict reload. This tests UI behavior only, not real authentication or deployed database integration. Real signed-in creator preview remains an activation gate.
