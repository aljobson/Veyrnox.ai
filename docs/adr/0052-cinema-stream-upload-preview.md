...
Browser evidence: local synthetic responses verified upload navigation, the 360px file/progress controls, ready-but-private messaging and session clearing. No video bytes were transmitted to Stream in this check. Existing Button/tokens/native controls were reused after 21st upload-form search; its deterministic review found no issues.

Issued direct-upload URLs are bearer capabilities until provider expiry. Membership revocation denies retrieving or issuing a grant but cannot invalidate a URL already copied by a client. Provider-side cancellation/revocation and cleanup must be designed before broader activation; the one-hour expiry and fixed size/duration/capacity bound this preview exposure.
