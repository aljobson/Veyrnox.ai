...
| [ADR-0026 — Cloudflare Turnstile as the CAPTCHA for Supabase Auth](0026-turnstile-captcha-on-auth.md) | **Accepted 2026-09-21** | Confirm email closed the credit faucet but not mail-quota burning. Turnstile tokens ride `gotrue_meta_security` on password sign-in, sign-up and magic link. CSP gains `challenges.cloudflare.com` in `script-src` and `frame-src` only while a site key is set; `connect-src` unchanged. Ship the client first, flip Supabase CAPTCHA second — the reverse order locks out every email user. | — |
