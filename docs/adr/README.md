...
| [ADR-0021 — Generated media moves to the EU-jurisdiction R2 bucket](0021-media-in-eu-jurisdiction-r2.md) | **Accepted 2026-09-13** (cutover blocked on target bucket and credentials) | `R2_JURISDICTION=eu` selects the EU S3 endpoint. Production's media is in `veyrnox-staging-media`; cutover copies live assets to EU `veyrnox-media` and sets `R2_BUCKET` and `R2_JURISDICTION` together. Meets ADR-0005 §3 for media. | — |
