...
| [ADR-0028 — The browser uploads start images straight to R2](0028-browser-upload-to-r2.md) | **Accepted 2026-09-22** | Image-input models were live but unusable: no client called `/api/v1/uploads`, `connect-src` blocked R2 and the bucket had no CORS. Direct PUT on the gateway-signed URL (no Worker proxy); `connect-src` widened by our own R2 endpoint only; bucket CORS `https://veyrnox.ai`, PUT, Content-Type; the create page shows the picker from `capabilities.media`. | ADR-0027 |
