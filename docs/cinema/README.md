...
Categories (0148, ADR-0059 addendum): `cinema_categories` holds the fixed genre list; a title carries one or two, required before submission; `GET /api/cinema/titles?category=<slug>` filters the catalogue and every list response carries `categories` for the tabs on `/social-cinema`.
