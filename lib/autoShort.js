 ...
 */
async function submitStep(jobId, step, ordinal, inputs, deps, opts = {}) {
    const spec = STEPS[step];
    if (!opts.retry) {
        // Claim the row before spending anything. UNIQUE (job_id, step,
        // ordinal) settles a race between a callback and the sweep, and the
        // loser never calls the provider (#230).
        const claim = await deps.rpc('job_step_claim', {
            p_job_id: jobId, p_step: step, p_ordinal: ordinal,
            p_provider: spec.provider, p_provider_endpoint: spec.endpoint,
        });
        if (!claim || claim.ok !== true) return { ok: false, error: String((claim && claim.code) || 'step_not_claimable').toLowerCase() };
...
export async function onStepOutcome({ step, outcome }, deps) {
    ...
    if (outcome.state !== 'success') {
        if (step.attempts < 2) {
            ...
            const again = inputs ? await submitStep(jobId, name, ordinal, inputs, deps, { retry: true }) : { ok: false };
