...
import { AI_DISCLOSURES, CATEGORY_SLUG, MAX_CATEGORIES } from './domain.js';
...
const errors = { creator_required: 403, account_not_active: 403, content_not_found: 404,
  invalid_draft: 400, invalid_category: 400, invalid_parent: 400, revision_conflict: 409, idempotency_conflict: 409,
  structure_locked: 409, position_taken: 409, draft_locked: 409, draft_limit_reached: 409 };
export function validDraft(body) {
  if (!body || Array.isArray(body) || typeof body !== 'object' || fields.some(k => !Object.hasOwn(body, k))
    || Object.keys(body).some(k => !fields.includes(k) && !optional.includes(k))
    || (Object.hasOwn(body, 'categories') && (!Array.isArray(body.categories) || body.categories.length > MAX_CATEGORIES
      || new Set(body.categories).size !== body.categories.length || body.categories.some(c => typeof c !== 'string' || !CATEGORY_SLUG.test(c))
      || (body.categories.length > 0 && ['SEASON','EPISODE'].includes(body.content_type)))) || !TYPES.includes(body.content_type)
