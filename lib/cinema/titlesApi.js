...
export function titlesHandler({ action = 'list', authed = false, rpcCall = rpc, cache = { get: cacheGet, put: cachePut } } = {}) {
  return async (req, ctx) => {
    ...
    try {
      ...
      if (action === 'list') {
        if ([...query.keys()].some((k) => !['before', 'limit', 'category'].includes(k))) return reply({ error: 'invalid_query' }, 400);
        const before = query.get('before');
        const category = query.get('category');
        const limit = query.has('limit') ? Number(query.get('limit')) : 24;
        if ((before !== null && (Number.isNaN(Date.parse(before)) || before.length > 40)) || !Number.isInteger(limit) || limit < 1 || limit > 50
          || (category !== null && !CATEGORY_SLUG.test(category))) return reply({ error: 'invalid_query' }, 400);
        const cacheKey = !authed ? `https://veyrnox.ai/api/cinema/titles?limit=${limit}&before=${before ? encodeURIComponent(new Date(before).toISOString()) : ''}&category=${category || ''}` : null;
        if (cacheKey) { const hit = await cache.get(cacheKey); if (hit) return hit; }
        const [titles, categories] = await Promise.all([
          rpcCall('list_public_cinema_titles', { p_limit: limit, p_before: before ? new Date(before).toISOString() : null, p_category: category }, cfg),
        ...
        const res = reply({ titles, categories, category, next: titles.length === limit ? titles[titles.length - 1].published_at : null }, 200, !authed);
