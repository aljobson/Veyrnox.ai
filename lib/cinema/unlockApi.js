...
const errors = {
  ...
  rate_limited: 429, invalid_seconds: 400,
};
...
export function unlockHandler({ action = 'entitlement', rpcCall = rpc, sign = signStreamPlayback, now = () => Date.now() } = {}) {
  return async (req) => {
    ...
    try {
      ...
      const allowed = action === 'unlock' ? ['content_id', 'consent_version'] : action === 'heartbeat' ? ['content_id', 'seconds'] : ['content_id'];
      ...
      if (action === 'heartbeat' && (!Number.isSafeInteger(body.seconds) || body.seconds < 1 || body.seconds > 60)) return reply({ error: 'invalid_request' }, 400);
      ...
      if (action === 'entitlement') {
        ...
        return reply({ access: ent.access, credits: ent.credits, content_id: body.content_id, consent_version: UNLOCK_CONSENT_VERSION, ...(ent.reason ? { reason: ent.reason } : {}) });
      }
      if (action === 'unlock') {
      ...
      if (action === 'heartbeat') {
        // A Pass Play (ADR-0057 Phase 3): only a Pass holder records seconds; everyone else gets recorded:false.
        const h = await rpcCall('record_cinema_pass_play', { ...args, p_seconds: body.seconds }, db);
        if (h?.error) return reply({ error: Object.hasOwn(errors, h.error) ? h.error : 'temporarily_unavailable' }, errors[h.error] || 503);
        if (h?.ok !== true || typeof h.recorded !== 'boolean') return reply({ error: 'temporarily_unavailable' }, 503);
        return reply({ recorded: h.recorded, access: h.access, seconds: h.seconds ?? 0, minutes_used: h.minutes_used ?? null, ceiling_minutes: h.ceiling_minutes ?? null, ...(h.reason ? { reason: h.reason } : {}), content_id: body.content_id });
      }
      // play: the token is minted only for an entitled viewer, bound to one video.
