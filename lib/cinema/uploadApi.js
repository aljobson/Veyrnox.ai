...
const errors={upload_not_allowed:403,invalid_upload:400,idempotency_conflict:409,upload_exists:409,upload_capacity_reached:429};
...
export function uploadHandler({action='read',rpcCall=rpc,createVideo=createStreamUpload,readVideo=readStreamVideo}={}) {
  return async req=>{
    ...
    try {
      ...
      const allowed=action==='start'?['content_id','file_size','fingerprint']:['content_id'];
      ...
      } else if(action==='refresh' && row?.stream_uid && !['ready','error'].includes(row.state)) {
