...
export function validateEdit({ clips, audio }) {
    ...
    return { ok: true, clips: out, audio: a, output_s: total };
}
...
async function submitStep(jobId, edit, next, rows, deps) {
...
export async function onStepOutcome({ step, outcome }, deps) {
    ...
    if (outcome.state !== 'success') {
        if (step.attempts < 2) {
            const again = await submitStep(jobId, edit, { step: name, ordinal }, await deps.steps(jobId), deps);
