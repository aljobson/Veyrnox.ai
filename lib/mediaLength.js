...
/**
 * MP4: walk the top-level boxes to `moov`, then read `mvhd` for the length
 * and the first `tkhd` with a picture size for the frame. `readRange`
...
function moovInfo(moov) {
    ...
    for (let i = 8; i + 32 <= moov.length; i += 1) {
        ...
        if (width === null && ascii(moov, i, 'tkhd')) {
            const at = i + (moov[i + 4] === 1 ? 92 : 80);
            if (at + 8 > moov.length) continue;
            const w = u32(moov, at) / 65536;
            const h = u32(moov, at + 4) / 65536;
            if (w > 0 && h > 0) { width = Math.round(w); height = Math.round(h); }
        }
    }
    return seconds === null ? null : { seconds, width, height };
}
