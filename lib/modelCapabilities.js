 ...
 *   media      slot -> { field, required, list, maxPixels }: reference URLs
 *              the model accepts; `list` sends the URL as a one-item array;
 *              `maxPixels` refuses a larger source image before the debit
 *   derive     optional (inputs, source) -> fields computed per request from
...
const FAL = {
    ...
    'fal-ai/bria/expand': {
        ...
        fixed: { sync_mode: false },
        assumes: {},
        derive: (inputs, source) => ({ canvas_size: expandCanvas(source.dimensions, inputs.aspect_ratio || '16:9') }),
        derives: ['canvas_size'],
    },
...
/**
 * Pre-debit check of an uploaded source against the model's slot: a slot
 * with `maxPixels` needs a readable size at or under it.
 * @param {{dimensions:{width:number,height:number}|null}|null} source
 * @returns {{ok:true}|{ok:false,error:string}}
 */
export function checkSource(record, source) {
    const spec = record && record.media && record.media.image;
    if (!spec || !spec.maxPixels || !source) return { ok: true };
 ...
 */
export function shapePayload(record, inputs, source = null) {
    const out = {};
    ...
    const derived = record.derive && source && source.dimensions ? record.derive(inputs, source) : {};
...
export function publicCapabilities(record) {
    ...
    const media = {};
    for (const [slot, spec] of Object.entries(record.media)) {
        media[slot] = spec.maxPixels ? { required: !!spec.required, maxPixels: spec.maxPixels } : { required: !!spec.required };
    }
