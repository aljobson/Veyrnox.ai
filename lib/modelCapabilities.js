 ...
 *
 * Step 1 is data plus pure helpers. Nothing on the request path reads this yet;
 * tests/modelCapabilities.test.mjs proves the helpers reproduce today's
 * lib/providerDuration.js payloads, and scripts/check-capabilities.mjs checks
...
const FAL = {
    ...
    // Today's providerDuration.js does NOT produce this; step 2 switches the
    // gateway to it before the row is reactivated.
...
export function checkInputs(record, inputs) {
    ...
    const mediaKeys = new Set(Object.keys(record.media).map((slot) => (slot === 'image' ? 'image_url' : `${slot}_url`)));
    ...
    for (const [slot, spec] of Object.entries(record.media)) {
        const key = slot === 'image' ? 'image_url' : `${slot}_url`;
        if (spec.required && !(inputs && inputs[key])) return { ok: false, error: `inputs_invalid:${key}` };
    }
    return { ok: true };
}
...
export function shapePayload(record, inputs) {
    ...
    for (const [slot, spec] of Object.entries(record.media)) {
        const key = slot === 'image' ? 'image_url' : `${slot}_url`;
        if (inputs[key] !== undefined) out[spec.field] = inputs[key];
    }
