 ...
 *
 * Record fields
 *   provider   'fal' | 'kie' | 'openrouter' | 'grsai' | 'veyrnox' (our own pipeline)
...
import { OTHERS } from './additionalModelCapabilities.js';
...
const FAL = {
    ...
    // Sana defaults to 4K: pin one sub-1MP image so 1 credit buys at most
    // $0.01 of generation. Separate option; FLUX's record is unchanged.
    'fal-ai/sana/v1.5/4.8b': {
        provider: 'fal', kind: 'image',
        inputs: { prompt: PROMPT, negative_prompt: NEGATIVE, seed: SEED },
        rename: {}, media: {}, lengths: null,
        fixed: {
            image_size: { width: 1024, height: 768 }, num_images: 1,
            num_inference_steps: 18, guidance_scale: 5,
            enable_safety_checker: true, sync_mode: false,
            output_format: 'png', style_name: '(No style)',
        },
        assumes: {},
    },
