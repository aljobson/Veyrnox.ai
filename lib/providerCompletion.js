...
export async function dedup(cfg, source, id, payload) {
    const res = await fetchWithTimeout(new URL('/rest/v1/webhook_events?on_conflict=source,external_id', cfg.supabaseUrl), {
        method: 'POST',
    ...
    const read = await fetchWithTimeout(new URL(
...
/**
 * @param {object} args
 * @param {string} args.source         'kie' | 'openrouter' | 'grsai' (jobs.provider and webhook_events.source)
 ...
 * @param {object} [args.r2cfg]         explicit Worker env config for scheduled invocations
 * @param {object} args.cfg            Supabase config
 * @returns {Promise<{status:number, body:object}>}
 */
export async function completeJob({ source, job, providerJobId, outcome, ext, copyOptions = {}, cfg, r2cfg = r2EnvConfig() }) {
    ...
    if (outcome.state === 'success') {
        ...
        const copy = await copyUrlToR2(outcome.outputUrl, key, r2cfg, { provider: source, ...copyOptions });
        if (!copy.ok) {
