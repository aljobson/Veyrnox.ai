...
import { ownsUploadKey, typeForKey, checkSniffed, imageDimensions, SNIFF_BYTES, DIMENSION_BYTES, ALLOWED_UPLOAD_TYPES } from './uploadSource.js';
import { fetchWithTimeout } from './fetchWithTimeout.js';
import { wavSeconds, mp3Seconds, mp4Seconds } from './mediaLength.js';
...
async function lengthOf(contentType, bytes, size, url) {
    ...
    if (contentType === 'audio/mpeg') return mp3Seconds(bytes, size);
    if (contentType !== 'video/mp4') return null;
    const readRange = async (start, end) => {
        const res = await fetchWithTimeout(url, { headers: { Range: `bytes=${start}-${end}` } }, SNIFF_TIMEOUT_MS);
    ...
    try {
        ...
        console.error('[source] mp4 length read failed:', err && err.message);
        return null;
    }
}
