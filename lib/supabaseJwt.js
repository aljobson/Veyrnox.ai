 ...
 */
function staleFallback(why) {
    if (jwksCache && Date.now() - jwksCache.fetchedAt < JWKS_MAX_STALE_MS) return jwksCache.byKid;
...
async function loadJwks(supabaseUrl, { force = false } = {}) {
    ...
    try {
        ...
        return staleFallback(`jwks fetch: ${err && err.message}`);
    }
    if (!res.ok) {
        return staleFallback(`jwks ${res.status}`);
    }
    let jwks;
    try {
        jwks = await res.json();
    } catch (err) {
        return staleFallback(`jwks parse: ${err && err.message}`);
    }
    const byKid = new Map();
    ...
    if (byKid.size === 0) {
        return staleFallback('no usable jwks keys');
    }
