...
import { normaliseOrder } from '../packages/adapters/lemonsqueezy.js';
...
export function backfillVerdict(row, order, opts) {
    ...
    if (o.status !== 'paid') return { skip: 'order not paid', anomaly: true };
...
export async function runBackfill(rows, deps) {
    ...
    for (const [i, row] of rows.entries()) {
        ...
        try {
            ...
            log(LOG, 'credit_top_up failed:', err && (err.status ?? err.message), 'top_up', row.top_up_id);
            out.retry++;
            continue;
        }
        if (res && res.ok === true) {
            ...
            log(LOG, 'credit_top_up gave no usable verdict:', res && res.code, 'top_up', row.top_up_id);
            out.retry++;
        }
    }
    return out;
