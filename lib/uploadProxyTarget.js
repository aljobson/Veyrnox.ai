...
function assertProdNarrowing(env) {
    // In production the S3 host allowlist alone is too broad — every public
    // bucket is reachable. Require operators to enumerate the specific hosts
    // (S3 buckets, presigned domains) their upload flow actually targets via
    // UPLOAD_PROXY_ALLOWED_HOSTS. See docs/UPLOAD_PROXY_CONFIG.md.
    if (env.NODE_ENV !== 'production') return;
    const raw = env.UPLOAD_PROXY_ALLOWED_HOSTS;
    if (!raw || raw.trim() === '') {
        throw new Error(
            'UPLOAD_PROXY_ALLOWED_HOSTS must be set in production. ' +
