...
// ponytail: add a type when a verified endpoint needs it, not before.
export const ALLOWED_UPLOAD_TYPES = {
    ...
    // Lip-sync speech (ADR-0028). Length, not size, is what is billed; the
    // gateway reads it from the headers (lib/mediaLength.js).
    'audio/mpeg': { maxBytes: 20 * 1024 * 1024, extension: 'mp3' },
    'audio/wav': { maxBytes: 20 * 1024 * 1024, extension: 'wav' },
};

/** The generation input an upload of this media type becomes. */
export function fieldForType(contentType) {
    if (contentType.startsWith('video/')) return 'video_url';
    if (contentType.startsWith('audio/')) return 'audio_url';
    return 'image_url';
}
...
export function sniffType(bytes) {
    ...
    // MP3: an ID3v2 tag, or straight into an MPEG audio frame (11 sync bits).
    if (ascii(b, 0, 'ID3') || (b[0] === 0xff && (b[1] & 0xe0) === 0xe0 && ((b[1] >> 1) & 3) === 1)) return 'audio/mpeg';
 ...
 * @returns {{contentType:string, field:'image_url'|'video_url'|'audio_url'}|null}
 */
export function typeForKey(key) {
    ...
    for (const [contentType, rule] of Object.entries(ALLOWED_UPLOAD_TYPES)) {
        ...
        return { contentType, field: fieldForType(contentType) };
    }
    return null;
}
