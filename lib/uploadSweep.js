...
export async function sweepConsumedUploads(r2cfg, dbcfg, { now = new Date(), find = select, remove = deleteObject } = {}) {
    const oldestSafeJob = new Date(now.getTime() - 16 * 60 * 1000).toISOString();
    const since = new Date(now.getTime() - CONSUMED_WINDOW_MINUTES * 60 * 1000).toISOString();
    let rows;
    try {
        rows = await find('jobs', {
            columns: 'id,inputs',
            filter: `state=in.(${TERMINAL_STATES})&created_at=lte.${encodeURIComponent(oldestSafeJob)}&updated_at=gte.${encodeURIComponent(since)}&inputs->source_keys=not.is.null&order=updated_at.desc`,
            limit: MAX_DELETES,
        }, dbcfg);
    } catch (err) {
