-- Per-account, fixed 60-second window for account and balance reads.
-- One row per user bounds storage. Counters are operational metadata, not money.
CREATE TABLE IF NOT EXISTS public.account_read_rate_limits (
    user_id UUID PRIMARY KEY REFERENCES public.users(id) ON DELETE CASCADE,
    window_started_at TIMESTAMPTZ NOT NULL,
    request_count INTEGER NOT NULL CHECK (request_count BETWEEN 1 AND 121)
);
ALTER TABLE public.account_read_rate_limits ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.account_read_rate_limits FORCE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.account_read_rate_limits FROM PUBLIC, anon, authenticated, service_role;

CREATE OR REPLACE FUNCTION public.consume_account_read_request(p_auth_id TEXT)
RETURNS JSONB
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
BEGIN
    ...
    INSERT INTO public.account_read_rate_limits AS r (user_id, window_started_at, request_count)
...
REVOKE ALL ON FUNCTION public.consume_account_read_request(TEXT) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.consume_account_read_request(TEXT) TO service_role;
