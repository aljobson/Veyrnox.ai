-- First profitable batch of Higgsfield monthly-price parity (ADR-0037).
...
BEGIN
    ...
    UPDATE public.model_catalog SET credits_5s = 1, updated_at = now()
    WHERE id = 'nano-banana-kie' AND active AND provider = 'kie'
      ...
      AND billing_seconds IS NULL AND credits_5s IN (1, 2);
    GET DIAGNOSTICS affected = ROW_COUNT;
    IF affected <> 1 THEN
    ...
    UPDATE public.model_catalog SET credits_5s = 6, updated_at = now()
    WHERE id = 'hailuo-02-kie' AND active AND provider = 'kie'
      ...
      AND billing_seconds = 6 AND credits_5s IN (6, 10);
    GET DIAGNOSTICS affected = ROW_COUNT;
    IF affected <> 1 THEN
