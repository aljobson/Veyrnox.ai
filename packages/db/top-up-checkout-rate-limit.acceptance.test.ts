...
describe('checkout attempt quota (0120)', { skip: !process.env.DATABASE_URL }, () => {
    let pool: pg.Pool;
    const users: string[] = [];
    ...
    before(async () => {
        ...
        for (const name of ['0037_free_credit_expiry.sql', '0038_free_credit_sweep_fixes.sql',
            '0071_signup_grant_on_email_confirmation.sql', '0120_top_up_checkout_rate_limit.sql', '0120_top_up_checkout_rate_limit.sql']) {
    ...
    const consume = async (auth: string) => (await one('SELECT public.consume_top_up_checkout_request($1) AS r', [auth])).r;
    ...
    it('allows exactly 20 requests across concurrent connections and isolates users', async () => {
        ...
        assert.equal((await one('SELECT request_count FROM public.top_up_checkout_rate_limits WHERE user_id = $1', [a.id])).request_count, 21);
    });
    it('denials do not extend the window and an expired window resets the same row', async () => {
        const u = await user();
        await consume(u.auth);
        await pool.query('UPDATE public.top_up_checkout_rate_limits SET request_count = 20 WHERE user_id = $1', [u.id]);
        const before = await one('SELECT window_started_at FROM public.top_up_checkout_rate_limits WHERE user_id = $1', [u.id]);
        assert.equal((await consume(u.auth)).code, 'RATE_LIMITED');
        await pool.query(await readFile(new URL('./schema/supabase/0120_top_up_checkout_rate_limit.sql', import.meta.url), 'utf8'));
        assert.equal((await consume(u.auth)).code, 'RATE_LIMITED');
        const after = await one('SELECT window_started_at FROM public.top_up_checkout_rate_limits WHERE user_id = $1', [u.id]);
        assert.deepEqual(after, before);
        await pool.query("UPDATE public.top_up_checkout_rate_limits SET window_started_at = clock_timestamp() - interval '61 seconds' WHERE user_id = $1", [u.id]);
        assert.deepEqual(await consume(u.auth), { ok: true });
        assert.equal((await one('SELECT request_count FROM public.top_up_checkout_rate_limits WHERE user_id = $1', [u.id])).request_count, 1);
    });
    it('unknown identities allocate no quota rows', async () => {
        const before = (await one('SELECT count(*) FROM public.top_up_checkout_rate_limits')).count;
        assert.deepEqual(await consume(randomUUID()), { ok: false, code: 'NOT_FOUND' });
        assert.equal((await one('SELECT count(*) FROM public.top_up_checkout_rate_limits')).count, before);
    });
    it('forces RLS and permits service-role RPC execution only', async () => {
        const table = await one("SELECT relrowsecurity, relforcerowsecurity FROM pg_class WHERE oid = 'public.top_up_checkout_rate_limits'::regclass");
        assert.equal(table.relrowsecurity, true);
        assert.equal(table.relforcerowsecurity, true);
        for (const role of ['anon', 'authenticated', 'service_role']) {
            assert.equal((await one("SELECT has_function_privilege($1, 'public.consume_top_up_checkout_request(text)', 'EXECUTE') AS p", [role])).p, role === 'service_role');
            for (const permission of ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'TRUNCATE']) {
                assert.equal((await one("SELECT has_table_privilege($1, 'public.top_up_checkout_rate_limits', $2) AS p", [role, permission])).p, false);
            }
        }
        const fn = await one("SELECT prosecdef, proconfig FROM pg_proc WHERE oid = 'public.consume_top_up_checkout_request(text)'::regprocedure");
