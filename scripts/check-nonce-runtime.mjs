...
const seen = new Set();
for (const path of ['/', '/pricing', '/presets', '/legal/terms', '/m/create', '/not-a-page', '/app', '/app/credits', '/app/account', '/app/create', '/app/library', '/auth/callback', '/app/not-a-page']) {
    for (let n = 0; n < 2; n++) {
        ...
        assert.equal(response.status, path.endsWith('/not-a-page') ? 404 : 200, path);
...
console.log('Whole-site HTTP nonce/cache probe passed; browser and authenticated journey checks are separate.');
