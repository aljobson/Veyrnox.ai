...
// Fresh provider UIDs each run: stream_uid is UNIQUE, so a fixed one fails the second run against the same database.
...
const attach=(id,streamUid=uid)=>val('SELECT public.attach_cinema_upload($1,$2,$3) AS value',[id,streamUid,`https://upload.cloudflarestream.com/${streamUid}`]);
const observe=(state,time=new Date().toISOString(),streamUid=uid,duration=42)=>val('SELECT public.observe_cinema_upload($1,$2,$3,$4,$5,$6) AS value',[streamUid,state,time,duration,1080,1920]);
...
try {
 // Applying 0137 twice proves it is idempotent. Roll it back: 0139 redefines these functions,
 // and the assertions below must exercise the bodies a full replay leaves behind, not 0137's.
 const migration=await readFile(new URL('../packages/db/schema/supabase/0137_cinema_stream_uploads.sql',import.meta.url),'utf8');await q('BEGIN');await c.query(migration);await c.query(migration);await q('ROLLBACK');
 ...
 assert.equal((await observe('ready',new Date().toISOString(),uid,601)).error,'invalid_observation');
 ...
 const items=[];for(let n=1;n<10;n++){const item=await draft(actors[0]),reserved=await reserve(actors[0],item.id);assert.equal(reserved.claimed,true);items.push(reserved);}
 ...
 // One provider video belongs to one reservation: the UNIQUE stream_uid constraint refuses a
 // cross-row attach, and the API maps every failed attach to upload_needs_reconciliation.
 await assert.rejects(attach(items[0].id),e=>e.code==='23505');assert.equal((await read(actors[0],items[0].content_id)).upload.stream_uid,null);
 ...
 // Postgres 16 reports ON DELETE RESTRICT as foreign_key_violation, 18 as restrict_violation.
 await assert.rejects(c.query('DELETE FROM public.cinema_profiles WHERE user_id=(SELECT id FROM public.users WHERE auth_id=$1)',[actors[0]]),e=>['23503','23001'].includes(e.code));
 ...
 console.log('Cinema uploads: replay, concurrent single provisioning, ownership, role/status/Auth denial, capacity, one video per reservation, terminal observations, RLS/ACL, private drafts and unchanged credits passed.');
}finally{await c.end();}
