...
test('child switches cannot bypass profile, subscription or upload prerequisites', () => {
  ...
  for (const key of ['profiles','creators','content','uploads','unlocks','publishing','monetisation','voting','comments','ppv','premieres']) assert.equal(cinemaFeatures(env)[key], false);
