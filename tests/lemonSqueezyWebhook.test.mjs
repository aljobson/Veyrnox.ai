...
test('normaliseOrder: returns our shape with a string Top-up id from custom data', () => {
    const res = normaliseOrder(order(), { top_up_id: TOP_UP_ID }, { expectTestMode: true });
...
test('normaliseOrder: pre-tax paid amount is subtotal less discount', () => {
    const res = normaliseOrder(order({ discount_total: 2500 }), { top_up_id: TOP_UP_ID }, { expectTestMode: true });
    assert.equal(res.order.paidCents, 0);
});

test('normaliseOrder: parses the cumulative refunded amount', () => {
    const res = normaliseOrder(order({ status: 'partial_refund', refunded_amount: 1250 }), { top_up_id: TOP_UP_ID }, { expectTestMode: true });
...
test('normaliseOrder: rejects a non-UUID or missing Top-up id', () => {
    for (const custom of [{ top_up_id: 'not-a-uuid' }, { top_up_id: 12 }, {}, null, undefined, { top_up_id: `${TOP_UP_ID}x` }]) {
        const res = normaliseOrder(order(), custom, { expectTestMode: true });
...
test('normaliseOrder: rejects a test-mode mismatch either way', () => {
    assert.deepEqual(normaliseOrder(order(), { top_up_id: TOP_UP_ID }, { expectTestMode: false }), { ok: false, error: 'test_mode mismatch' });
    assert.deepEqual(
        normaliseOrder(order({ test_mode: false }, { test_mode: false }), { top_up_id: TOP_UP_ID }, { expectTestMode: true }),
        { ok: false, error: 'test_mode mismatch' },
    );
    assert.deepEqual(
        normaliseOrder(order({ test_mode: false }), { top_up_id: TOP_UP_ID }, { expectTestMode: false }),
        { ok: false, error: 'test_mode mismatch' },
        'either flag set means test mode',
    );
    assert.equal(
        normaliseOrder(order({ test_mode: false }, { test_mode: false }), { top_up_id: TOP_UP_ID }, { expectTestMode: false }).ok,
        true,
    );
});
...
test('normaliseOrder: rejects a malformed order', () => {
    ...
    assert.equal(normaliseOrder(null, custom, { expectTestMode: true }).ok, false);
    assert.equal(normaliseOrder({ id: 'abc', attributes: order().attributes }, custom, { expectTestMode: true }).ok, false);
    assert.equal(normaliseOrder(order({ subtotal: '2500' }), custom, { expectTestMode: true }).ok, false);
    assert.equal(normaliseOrder(order({}, { variant_id: null }), custom, { expectTestMode: true }).ok, false);
    assert.equal(normaliseOrder(order({ first_order_item: null }), custom, { expectTestMode: true }).ok, false);
});
